---
name: ares-scan
description: Web application vulnerability assessment with Ares MCP (crawl, fuzz, nuclei, security headers, CORS, gated injection tests). Use for authorized web app security testing.
category: security
mcp_server: ares
---
...
Use `playbook_web_app_test` or `playbook_vuln_triage` via `run_tool`:
```
target: https://<host>
```
...
- `crlf_test`, `host_header_inject`, `open_redirect_test`
